...
async function init() {
  ...
  startShakingHeadAndTalking(0.5, 1.1, 0.9); // 中等强度，中等速度，嘴巴中等张开
...
function updateHeadMotion(model: Live2DModel) {
  ...
  for (let param of headParams) {
    ...
    model.angle = -headMotion.currentAngle / 100;
  }
}
...
/**
 * 让人物开始摇头说话
 * @param intensity 动作强度 (0-1)
 * @param speed 摇头速度 (0-1)
 * @param mouthOpen 嘴巴张开程度 (0-1)
 */
function startShakingHeadAndTalking(
  intensity: number = 0.8,
  speed: number = 0.5,
  mouthOpen: number = 0.7
) {
  if (!currentModel) return;
  // 设置摇头参数
  headMotion.maxAngle = 15 * intensity;
  headMotion.returnSpeed = 0.1 + (1 - speed) * 0.4;
  headMotion.isReturning = false;
  headMotion.beatInProgress = false;

  // 设置嘴巴参数
  mouthMotion.maxOpen = mouthOpen;
  mouthMotion.decayRate = 0.1 + (1 - speed) * 0.2;
  console.log(mouthMotion);

  // 开始周期性摇头
  const shakeInterval = setInterval(() => {
    if (!headMotion.beatInProgress) {
      headMotion.direction *= -1;
      headMotion.targetAngle = headMotion.maxAngle * headMotion.direction;
      mouthMotion.targetOpen = mouthMotion.maxOpen;
    }
  }, 500 / speed);

  // 保存interval以便停止
  (currentModel as any)._shakeInterval = shakeInterval;
}

/**
 * 停止人物摇头说话的动作
 */
function stopShakingHeadAndTalking() {
  if (!currentModel) return;
  // 清除摇头定时器
  const shakeInterval = (currentModel as any)._shakeInterval;
  if (shakeInterval) {
    clearInterval(shakeInterval);
    delete (currentModel as any)._shakeInterval;
  }
